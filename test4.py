import cv2
import pyrect
import numpy as np

array = []

for

j=0

def main():

    video_path = 'D:\krk.mp4'
    cv2.ocl.setUseOpenCL(False)

    #read video file
    cap = cv2.VideoCapture(video_path)

    ret, frame1 = cap.read()
    ret, frame2 = cap.read()

    while ret:
        i = 1
        _, frame = cap.read()

        d = cv2.absdiff(frame1, frame2)

        imgray = cv2.cvtColor(d, cv2.COLOR_BGR2GRAY)
        blur = cv2.GaussianBlur(imgray, (1, 1), 0)
        ret, thresh = cv2.threshold(blur, 20, 255, cv2.THRESH_BINARY)
        thresh = cv2.dilate(thresh, None, iterations=2)
        _, contours, h = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        for c in contours:
            if cv2.contourArea(c) < 200 or cv2.contourArea(c) > 400:
                continue
            # if j==0:
            #     (x, y, w, h) = cv2.boundingRect(c)
            #     array[i] = pyrect.Rect(x,y,w,h)
            #     i=i+1
            # else:
            #     (x, y, w, h) = cv2.boundingRect(c)
            #     if abs(array[i].left-x) < 20


            #get bounding box from countour
            (x, y, w, h) = cv2.boundingRect(c)
            array[i] = pyrect.Rect(x, y, w, h)

            # draw bounding box
            #cv2.rectangle(frame1, (x, y), (x + w, y + h), (0, 255, 0), 2)

            cv2.rectangle(frame1, (array[i].left, array[i].right), (array[i].left + array[i].width, array[i].right + array[i].height), (0, 255, 0), 2)
            i=i+1

        cv2.imshow("inter", frame1)

        if cv2.waitKey(40) == 27:
            break

        frame1 = frame2
        ret, frame2 = cap.read()
        # j=j+1
    cv2.destroyAllWindows()
    cap.release()


main()